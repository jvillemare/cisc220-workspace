...
class hashNode {
	...
	string *values; /*the dynamically allocated array (not linked list!) of
		words that follow the key in the Dr. Seuss text. If this set of words
		gets to be larger than the valuesSize of the array, then you must
		re-allocate the array double in size and copy over all the old values */
...
public:
	...
	hashNode(string s); /* initializes keyword to s, the valuesSize to 100
	...
	void dblArray(); /* creates a new array, double the length, and copies over
